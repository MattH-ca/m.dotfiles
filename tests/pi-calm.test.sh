...
# - real Pi TUI proofs in tmux without credentials or provider calls.
set -u
...
# Pi is installed from Homebrew here, not as an npm global (home.nix explains
# why), so look under the brew prefix first and fall back to the npm global
# root. `brew --prefix <formula>` is a version-independent symlink, so this
# keeps resolving across Pi's frequent releases.
resolve_pi_package_dir() {
  local candidate
  for candidate in \
    "$(brew --prefix pi-coding-agent 2>/dev/null)/libexec/lib/node_modules/@earendil-works/pi-coding-agent" \
...
PI_PACKAGE_DIR=${PI_CALM_TEST_PACKAGE_DIR:-$(resolve_pi_package_dir || true)}

# The smoke assertions below are the real proof, so they run against whatever
# Pi is installed: a genuine TUI regression should fail on a named assertion,
# not on a version string. Homebrew tracks Pi's releases closely (0.82 to 0.84
# inside one week), so gating on an exact version only produced false failures.
# This records what the proofs were last verified against; update it after
# re-running them on a new Pi.
PI_SMOKE_VERIFIED_VERSION=0.84.0
...
test_zero_coupling_and_state_file() {
  ...
  # Fork deviation: this repo intentionally runs First Mate, so README.md and
  # home.nix reference it legitimately. Only the Calm extension is scanned,
  # which preserves the real assertion: Calm itself has no upstream coupling.
...
test_static_typescript_and_repo_wiring() {
  # Reported in the pass line so a skipped typecheck is never announced as a
  # passing one.
  local typecheck_note="TypeScript typechecks"
  ...
  if ! have_pi_package; then
    ...
    typecheck_note="TypeScript check skipped"
...
JSON
  ...
  pass "static wiring: Home Manager auto-load intact, $typecheck_note, existing JS extension parses"
}
...
test_real_pi_tui_smoke() {
  local fixture agent project socket pane i pi_version
  if ! command -v pi >/dev/null 2>&1 || ! command -v tmux >/dev/null 2>&1; then
    echo "skip: pi or tmux not found for isolated real TUI smoke"
    return 0
  fi
  pi_version=$(pi --version 2>/dev/null || true)
  [ -n "$pi_version" ] || fail "real Pi smoke could not read 'pi --version'"
  if [ "$pi_version" != "$PI_SMOKE_VERIFIED_VERSION" ]; then
    printf 'note: Pi %s differs from the last verified proof target %s; running the smoke anyway\n' \
      "$pi_version" "$PI_SMOKE_VERIFIED_VERSION" >&2
  fi
