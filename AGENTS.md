...
- Never run `no-mistakes daemon start` or `daemon restart`; both silently break the pipeline's GitHub access. The daemon is started in the foreground from a herdr pane via `bin/no-mistakes-daemon`. The herdr plugin `herdr-plugins/nms-autostart` restarts it in an "NMS" tab automatically after each herdr session restore; manually, run the `nms` alias in a herdr pane. Do not try to move the daemon back to launchd - the full investigation of why that cannot work is in the private companion notes.
