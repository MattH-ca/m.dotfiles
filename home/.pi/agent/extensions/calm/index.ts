...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    // Pi's TerminalInputHandler may return { consume, data } to swallow or
    // rewrite the keystroke. Calm only observes input, so every path returns
    // undefined; the annotation states that so a future contract change fails
    // the typecheck here rather than silently changing what Calm intercepts.
